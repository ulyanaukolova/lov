...
namespace ULLLL
{
    public partial class AdminWindow : Window
    {
        ...
        private void LoadOrdersForOrderHistory()
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                connection.Open();
                ...
                OrderHistoryOrderComboBox.ItemsSource = ordersTable.DefaultView;
                OrderHistoryOrderComboBox.DisplayMemberPath = "OrderDate"; 
                OrderHistoryOrderComboBox.SelectedValuePath = "Order_ID"; 
            }
        }
        ...
        private void LoadOrderHistory()
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                OrderHistoryDataGrid.ItemsSource = orderHistoryTable.DefaultView;
            }
        }
        ...
        private void AddOrderHistoryButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                // Проверяем, что все необходимые данные введены
                if (OrderHistoryOrderComboBox.SelectedItem == null ||
                    string.IsNullOrEmpty(OrderHistoryStatusTextBox.Text) ||
                    string.IsNullOrEmpty(OrderHistoryUpdatedStatusTextBox.Text))
                {
                    MessageBox.Show("Пожалуйста, заполните все поля.", "Ошибка", MessageBoxButton.OK, MessageBoxImage.Error);
                    return;
                }

                int orderId = Convert.ToInt32(((DataRowView)OrderHistoryOrderComboBox.SelectedItem)["Order_ID"]);
                string status = OrderHistoryStatusTextBox.Text;
                DateTime updatedStatus = DateTime.Parse(OrderHistoryUpdatedStatusTextBox.Text);

                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    connection.Open();

                    // SQL-запрос для добавления записи в историю заказов
                    string query = @"
                INSERT INTO OrderHistory (Order_ID, Status, UpdatedStatus) 
                VALUES (@Order_ID, @Status, @UpdatedStatus)";
                    ...
                    if (rowsAffected > 0)
                    {
                        ...
                        LoadOrderHistory(); // Метод для обновления DataGrid после добавления
                    }
                    else
                    {
        ...
        private void DeleteOrderHistoryButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                // Получаем выбранную строку из DataGrid
                DataRowView selectedRow = (DataRowView)OrderHistoryDataGrid.SelectedItem;

                if (selectedRow == null)
                {
                    MessageBox.Show("Пожалуйста, выберите запись для удаления.", "Информация", MessageBoxButton.OK, MessageBoxImage.Information);
                    return;
                }

                int orderHistoryId = Convert.ToInt32(selectedRow["OrderHistory_ID"]);

                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    connection.Open();

                    // SQL-запрос для удаления записи истории заказов
                    string query = "DELETE FROM OrderHistory WHERE OrderHistory_ID = @OrderHistory_ID";

                    SqlCommand command = new SqlCommand(query, connection);
                    command.Parameters.AddWithValue("@OrderHistory_ID", orderHistoryId);
                    ...
                    if (rowsAffected > 0)
                    {
                        ...
                        LoadOrderHistory(); // Метод для обновления DataGrid после удаления
                    }
                    else
                    {
        ...
        private void UpdateOrderHistoryButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                DataRowView selectedRow = (DataRowView)OrderHistoryDataGrid.SelectedItem;
                ...
                if (selectedRow == null)
                {
                    MessageBox.Show("Пожалуйста, выберите запись для обновления.", "Информация", MessageBoxButton.OK, MessageBoxImage.Information);
                    return;
                }
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    string query = @"
                UPDATE OrderHistory 
                SET Order_ID = @Order_ID, Status = @Status, UpdatedStatus = @UpdatedStatus
                WHERE OrderHistory_ID = @OrderHistory_ID";
                    ...
                    if (rowsAffected > 0)
                    {
                        ...
                        LoadOrderHistory(); // Метод для обновления DataGrid после обновления
                    }
                    else
                    {
        ...
        {
            if (OrderHistoryDataGrid.SelectedItem != null)
            {
                DataRowView row = OrderHistoryDataGrid.SelectedItem as DataRowView;

                // Заполнение ComboBox с заказами
                OrderHistoryOrderComboBox.SelectedValue = row["Order_ID"];

                // Заполнение текстовых полей
                OrderHistoryStatusTextBox.Text = row["Status"].ToString();
                OrderHistoryUpdatedStatusTextBox.Text = row["UpdatedStatus"].ToString();
            }
        }
        ...
        private void SearchOrderHistoryButton_Click(object sender, RoutedEventArgs e)
        {
            string searchText = SearchOrderHistoryTextBox.Text.Trim();

            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                connection.Open();
                string query = "SELECT * FROM OrderHistory WHERE Status LIKE @SearchText";
                SqlCommand command = new SqlCommand(query, connection);
                ...
                OrderHistoryDataGrid.ItemsSource = searchResultsTable.DefaultView;
            }
        }


        private void SearchOrderHistoryTextBox_TextChanged(object sender, TextChangedEventArgs e)
        {
            string searchText = SearchOrderHistoryTextBox.Text.Trim();
     ...
     private void SearchOrders()
        ...
        private void OrderHistorySortComboBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            FilterAndSortOrderHistory();
        }

        private void FilterAndSortOrderHistory()
        {
            string searchText = SearchOrderHistoryTextBox.Text.Trim();

            // Получаем выбранные параметры сортировки
            string sortColumn = ((ComboBoxItem)OrderHistorySortColumnComboBox.SelectedItem)?.Tag.ToString() ?? "UpdatedStatus";
            string sortDirection = ((ComboBoxItem)OrderHistorySortDirectionComboBox.SelectedItem)?.Tag.ToString() ?? "ASC";

            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                connection.Open();
                string query = $@"
        SELECT OrderHistory_ID, Order_ID, Status, UpdatedStatus
        FROM OrderHistory
        WHERE Status LIKE @SearchText
        OR UpdatedStatus LIKE @SearchText
        ORDER BY {sortColumn} {sortDirection}";
